...
const logAction = async ({ userId, action, entityType, entityId, oldData, newData }) => {
    ...
    const logEntry = new ActionLog({
      ...
      entityId,
      changes: {
        before: Object.keys(filteredOldData).length > 0 ? filteredOldData : undefined,
        after: Object.keys(filteredNewData).length > 0 ? filteredNewData : undefined,
        diff,
      },
    });
