...
supplier (необязательный) — фильтрация по поставщику (по частичному совпадению).  **При передаче параметров через запятую отдает данные с учетом мулитивыбора**  
description (необязательный) — фильтрация по описанию (по частичному совпадению).  
status (необязательный) — фильтрация по статусу материала (Active и т.д.).  **При передаче параметров через запятую отдает данные с учетом мулитивыбора**  
partNumber (необязательный) — фильтрация по частичному совпадению номера материала.  **При передаче параметров через запятую отдает данные с учетом мулитивыбора**  
countryOfOrigin (необязательный) — фильтрация по стране происхождения (по частичному совпадению).  **При передаче параметров через запятую отдает данные с учетом мулитивыбора**  
