...
The synthetic ARIA grid used by tests is in `fixtures/synthetic-grid.html`. It covers partial visibility, horizontal accumulation, row changes, hidden columns, and duplicate-safe ordinals without using the real confidential sheet. The Playwright runner loads the unpacked extension, serves this fixture at an allowlisted Smartsheet URL, uses a local mocked ECN API, verifies RU/EN, DOM and TSV analysis, draft copying, and asserts that capture did not mutate the grid. It opens the panel document directly because Playwright cannot reliably click Chrome's toolbar action; the real action-host gate is covered by the background implementation and manifest review.
