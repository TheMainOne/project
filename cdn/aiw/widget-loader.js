...
(function () {
  ...
  const host =
    s.getAttribute("data-host") ||              // можно задать руками
    (scriptUrl ? scriptUrl.origin : "") ||      // обычный случай
  ...
  // jsSrc можно переопределить, но по умолчанию берём относительно host
  const jsSrc =
  ...
  if (isInline) {
    ...
    // --- ищем контейнер, максимально просто для клиента ---
    ...
    // --- fit=container → высота равна высоте контейнера ---
  ...
  (async () => {
    ...
    try {
      ...
      // сервер может вернуть либо { config: {...} }, либо уже готовый объект
    ...
    const cfg = {
      ...
      siteId,
      clientId: clientId || config?.clientId || null,
